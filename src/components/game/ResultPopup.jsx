...
export default function ResultPopup({
  ...
  // Prefer explicit solutionExpr, then server-canonical problemRef, then fallback string
  const resolvedSolution =
    solutionExpr ||
    (problemRef && problemRef.current && problemRef.current.solutionExpr) ||
    "No solution";
      ...
      {resultPopup === "wrong" && (
        <>
          ...
          <p className="solution-text">
            💡 {T.solution}: <br />
            <span className="solution-highlight">{resolvedSolution}</span>
          </p>
        </>
      )}
