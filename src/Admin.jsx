...
export default function Admin() {
    ...
    const currentTheme = useMemo(
        () =>
            ({
                galaxyBlue: {
                    background:
                        "radial-gradient(circle at 20% 30%, #001133, #000000 70%)",
                    text: "#eaf6ff",
                },
                galaxyPink: {
                    background:
                        "radial-gradient(circle at 80% 20%, #2a001f, #000000 80%)",
                    text: "#ffe6ff",
                },
            })[theme],
        [theme]
    ...
    const startRoom = async (modeKey) => {
        if (
            !confirm(
                `Force-start room "${modeKey}"? The server will pick the first player.`
            )
        )
            return;
        try {
                            ...
                            >
                                {roomKeys.map((modeKey) => {
                                    ...
                                    const players = Array.isArray(r.players)
                                        ? r.players
                                        : [];
                                    const turnOrder = Array.isArray(r.turnOrder)
                                        ? r.turnOrder
                                        : [];
