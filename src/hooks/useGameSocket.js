...
export default function useGameSocket({
    ...
    useEffect(() => {
        ...
        socket.on("yourTurn", ({ mode: serverMode }) => {
            ...
            const hasServerProblem = !!(
                problemRef.current &&
                problemRef.current.target !== undefined &&
                problemRef.current.target !== null
            );
            if (hasServerProblem) {
                console.log(
                    now(),
                    "ℹ️ Server problem already present — not generating locally.",
                    problemRef.current
                );
                // turn UI on
                setIsMyTurn(true);
            ...
            console.log(
                now(),
                "⚠️ No server problem found — generating fallback locally"
            );
